...
#define FUNC_DECLARATION "\\s*(int|void)\\s*[A-Za-z_]\\w*\\s*\\(\\s*((int|string|(char\\s*\\*)|char)\\s*[A-Za-z_]\\w*\\s*)?(,\\s*(int|string|char|(char\\s*\\*))\\s*[A-Za-z_]\\w*\\s*)*\\)"
...
string func_call(string func_expression);
